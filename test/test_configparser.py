...
class Support:
    ...
    def __init__(self):
        ...
        self.TESTFN = os.path.join(self.testdir, "testcfgfile.ini")
    ...
    def test_case_sensitivity_mapping_access(self):
        ...
        for opt in ("a-b", "A-b", "a-B", "A-B"):
            self.assertTrue(
                opt in cf["A"],
                "has_option() returned false for option which should exist")
        eq(set(cf["A"].keys()), {"a-b"})
        eq(set(cf["a"].keys()), {"b"})
        del cf["a"]["B"]
        eq(len(cf["a"].keys()), len({}))

        # SF bug #432369:
        cf = self.fromstring(
            "[MySection]\nOption{} first line   \n\tsecond line   \n".format(
                self.delimiters[0]))
        eq(set(cf["MySection"].keys()), {"option"})
...
class RawConfigParserTestCase(BasicTestCase, unittest.TestCase):
    ...
    def test_set_nonstring_types(self):
        ...
        if cf._dict is collections.OrderedDict:
    ...
    def test_inheritance(self):
        class StrangeConfigParser(configparser.ConfigParser):
            ...
            def getboolean(self, section, option, raw=False, vars=None,
                ...
                if section == option:
                    return True
                return super(StrangeConfigParser, self).getboolean(section, option, raw=raw, vars=vars,
                                          fallback=fallback)
